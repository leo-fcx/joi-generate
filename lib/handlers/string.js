...
var StringTypeHandler = function (opts) {
    ...
    var Email = function(data, schema, next, exit) {
        ...
        if (test) {
            ...
            data.invalid = data.valid.replace(emailRegExp, '$1$2invalid');
            data.empty = data.valid.replace(emailRegExp, '$2$3');
            data.long = undefined;
            data.short = undefined;
            data.bogus = data.valid.replace(emailRegExp, STRING.BOGUS + '$2$3');
    ...
    var Guid = function (data, schema, next, exit) {
        ...
        if (test) {
            ...
            data.valid = uuid.v4();
            data.invalid = 'zzzzzzzz-zzzz-zzzz-zzzz-zzzzzzzzzzzz';
    ...
    var MaxLength = function (data, schema, next) {
        ...
        if (max) {
            ...
            data.valid = msg.substr(0, goTo);
            data.long = 'Long ' + chance.sentence({words: max}).substr(0, max);
            data.bogus = chance.string({
                symbols: true,
                length: goTo
            });
    ...
    var RegEx = function (data, schema, next) {
        ...
        if (regex) {
            ...
            // TODO: RandExp not always generates a valid string for the regex
            do {
                data.valid = new RandExp(regex).gen();
            }
            while (!regex.test(data.valid));
            data.short = undefined;
            data.long = undefined;
            data.bogus = chance.string({
                symbols: true,
    ...
    var UpperCase = function (data, schema, next) {
        ...
        if (uppercase) {
            data.valid = data.valid && data.valid.toUpperCase();
            data.long = data.long && data.long.toUpperCase();
            data.short = data.short && data.short.toUpperCase();
        }
        next(null, data, schema);
    };
    ...
    var LowerCase = function (data, schema, next) {
        ...
        if (lowercase) {
            data.valid = data.valid && data.valid.toLowerCase();
            data.long = data.long && data.long.toLowerCase();
            data.short = data.short && data.short.toLowerCase();
        }
        next(null, data, schema);
    };
